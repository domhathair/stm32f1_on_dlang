...
enum {
    __STDC_VERSION_STDATOMIC_H__ = 202_311,
...
A atomic_exchange_explicit_impl(A, C)(shared(A)* obj, C desired, memory_order order) @trusted {
    ...
    case memory_order.memory_order_relaxed:
        return atomicExchange!(
            memory_order.memory_order_relaxed)(cast(A*)obj, cast(A)desired);

    case memory_order.memory_order_acquire:

        return atomicExchange!(
            memory_order.memory_order_seq_cst)(cast(A*)obj, cast(A)desired);

    case memory_order.memory_order_release:
        return atomicExchange!(
            memory_order.memory_order_release)(cast(A*)obj, cast(A)desired);

    case memory_order.memory_order_acq_rel:
        return atomicExchange!(
            memory_order.memory_order_acq_rel)(cast(A*)obj, cast(A)desired);
...
@trusted unittest {
    ...
    static assert(__traits(compiles, {
            bool __b;
            b = atomic_compare_exchange_weak_impl(&obj, &expected, 2);
        }));
}
...
@trusted unittest {
    ...
    static assert(__traits(compiles, {
            bool __b;
            b = atomic_compare_exchange_weak_impl(&obj, &expected, 2.1);
        }));
}

pragma(inline, true)
bool atomic_compare_exchange_strong_explicit_impl(A, B, C)(shared(A)* obj, B* expected, C desired,
    memory_order succ, memory_order fail) @trusted {
    static assert(is(shared(B) : A), "Both expected and object must be the same type");
    assert(obj !is null);

    final switch (succ) {
    case memory_order.memory_order_relaxed:
        return atomicCompareExchangeStrong!(memory_order.memory_order_relaxed, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    case memory_order.memory_order_acquire:
        return atomicCompareExchangeStrong!(memory_order.memory_order_acquire, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    case memory_order.memory_order_release:
        return atomicCompareExchangeStrong!(memory_order.memory_order_release, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    case memory_order.memory_order_acq_rel:
        return atomicCompareExchangeStrong!(memory_order.memory_order_acq_rel, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    case memory_order.memory_order_seq_cst:
        return atomicCompareExchangeStrong!(memory_order.memory_order_seq_cst, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    }
}
...
@trusted unittest {
    shared(int) obj = 3;
    int expected = 3;
    assert(atomic_compare_exchange_strong_explicit_impl(&obj, &expected, 2, memory_order.memory_order_seq_cst,
...
@trusted unittest {
    ...
    size_t[2] toSwap = [1, 2];
    assert(atomic_compare_exchange_strong_explicit_impl(&obj, &expected, toSwap, memory_order.memory_order_seq_cst,
            memory_order.memory_order_seq_cst));
}
...
@trusted unittest {
    shared(float) obj = 3;
    float expected = 3;
    assert(atomic_compare_exchange_strong_explicit_impl(&obj, &expected, 2.1, memory_order.memory_order_seq_cst,
            memory_order.memory_order_seq_cst));
}

pragma(inline, true)
bool atomic_compare_exchange_weak_explicit_impl(A, B, C)(shared(A)* obj, B* expected, C desired, memory_order succ,
    memory_order fail) @trusted {
    static assert(is(shared(B) : A), "Both expected and object must be the same type");
    assert(obj !is null);

    final switch (succ) {
    case memory_order.memory_order_relaxed:
        return atomicCompareExchangeWeak!(memory_order.memory_order_relaxed, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    case memory_order.memory_order_acquire:
        return atomicCompareExchangeWeak!(memory_order.memory_order_acquire, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    case memory_order.memory_order_release:
        return atomicCompareExchangeWeak!(memory_order.memory_order_release, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    case memory_order.memory_order_acq_rel:
        return atomicCompareExchangeWeak!(memory_order.memory_order_acq_rel, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    case memory_order.memory_order_seq_cst:
        return atomicCompareExchangeWeak!(memory_order.memory_order_seq_cst, memory_order
                .memory_order_relaxed)(cast(B*)obj, expected, cast(B)desired);
    }
}
...
@trusted unittest {
    ...
    int expected = 3;
    bool __b;
    __b = atomic_compare_exchange_weak_explicit_impl(&obj, &expected, 2, memory_order.memory_order_seq_cst, memory_order
...
@trusted unittest {
    ...
    size_t[2] expected = [3, 4];
    size_t[2] toSwap = [1, 2];
    assert(atomic_compare_exchange_weak_explicit_impl(&obj, &expected, toSwap, memory_order.memory_order_seq_cst,
            memory_order.memory_order_seq_cst));
}
